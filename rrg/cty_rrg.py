...
# === Configuration ===
# Core Developed Markets (Checked by default)
CORE_TICKERS = [
    "SPY", "EWJ", "EWU", "EWG", "EWQ", "EWC", "EWA", "EWL", "EWI", "EWP", "EWH", "EWD"
]
...
def build_plotly_figure(graph_data, sorted_tickers):
    ...
    fig.update_layout(
        title=dict(text=f"Global Macro Country Rotation (vs {BENCHMARK})", font=dict(size=18, color="#ffffff")),
...
def generate_dashboard(df, graph_html, core_tickers, extended_tickers, filename):
    ...
    checkboxes_html = ""
    # Add Core Tickers (Checked by default)
    for ticker in core_tickers:
        checkboxes_html += f'<label class="cb-label"><input type="checkbox" class="ticker-cb" value="{ticker}" checked> {ticker}</label>\n'
    # Add Extended Tickers (Unchecked by default)
    ...
    <head>
    <meta charset="utf-8">
    <title>Global Macro Rotation Architecture</title>
    <style>
    ...
    <body>
        <div class="container">
            <div class="header-bar">
                <h2>Global Macro Alpha Matrix</h2>
        ...
        <script>
            window.addEventListener('load', function() {
                ...
                // Tag HTML Table Rows with data attributes
                ...
                // Update UI state based purely on checked boxes
                function updateVisibility(activeTickers) {
                    // Update Plotly
                    if(plotDiv && plotDiv.data) {
                        const visibilityUpdate = plotDiv.data.map(trace => {
                            return activeTickers.includes(trace.name) ? true : false;
                        });
                ...
                // Handle initial state (URL params override default checkbox state)
                const urlParams = new URLSearchParams(window.location.search);
                const urlTickersParam = urlParams.get('tickers');
                
                if (urlTickersParam !== null) {
                    const urlActiveTickers = urlTickersParam ? urlTickersParam.split(',').map(t => t.trim().toUpperCase()) : [];
                    checkboxes.forEach(cb => {
                        cb.checked = urlActiveTickers.includes(cb.value);
                    });
                }
                
                // Read the final state of checkboxes to initialize the view
                let initialActiveTickers = Array.from(document.querySelectorAll('.ticker-cb:checked')).map(box => box.value);
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(final_html)
    print(f"Global Dashboard deployed successfully at: {filename}")
...
if __name__ == "__main__":
    ...
    generate_dashboard(final_table, graph_html, CORE_TICKERS, EXTENDED_UNIVERSE, "rrg/cty_rrg_dashboard.html")